...
func _init():
	...
	for i in COLOR_NAMES.size():
		color = "#" + COLOR_NAMES[i][0]
		test_color = Color(color)
		#rgb = rgb(color)
		#hsl = hsl(color)
		COLOR_NAMES[i].append_array([test_color.r8, test_color.g8, test_color.b8, int(round(test_color.h * 359)), int(round(test_color.s * 100)),  int(round(test_color.v * 100))])
...
func name(color):
	...
	for i in COLOR_NAMES.size():
		...
		dv = abs(v - COLOR_NAMES[i][8]) / 255.0
	...
	return ["#" + COLOR_NAMES[cl][0], COLOR_NAMES[cl][1], shadergb(COLOR_NAMES[cl][2]),COLOR_NAMES[cl][2], false]
...
func rgb(color):
	var test_color = Color("#" + color)
	return [test_color.r8, test_color.b8, test_color.g8]

func shadergb(shadename):
	for i in COLOR_SHADES.size():
		if (shadename == COLOR_SHADES[i][1]):
			return "#" + COLOR_SHADES[i][0]
